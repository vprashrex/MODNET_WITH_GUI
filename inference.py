...
class BGRemove():
    ...
    def post_process(self, mask_data, background=False, backgound_path='background.jpg'):
        ...
        if background:
            ...
            back_image = cv2.resize(back_image, (width, height), cv2.INTER_AREA)
        else:
            back_image = np.full(self.original_im.shape,255.0)

        self.alpha = np.uint8(matte[:, :, 0]*255.0)
        cv2.imwrite('alpha.jpg',self.alpha)

...
class BGRemove():
    ...
    def post_process(self, mask_data,  backgound_path=None):
        ...
        if backgound_path:
            ...
            back_image = cv2.imread(backgound_path)
            back_image = cv2.resize(
                back_image, (width, height), cv2.INTER_AREA)
        else:
            back_image = np.full(self.original_im.shape, 255.0)

        self.alpha = np.uint8(matte[:, :, 0]*255.0)
        cv2.imwrite('apha.jpg', self.alpha)
        matte = matte * self.original_im + (1 - matte) * back_image
        return matte

    def image(self, filename, background=False, output='output/',save=True,image_name=None):
        ...
        if save:
            ...
            if image_name is None:
                ...
                return self.save(matte,output,background,image_name)
        else:
            h, w, _ = matte.shape
    ...
    def save(self, matte, output_path='output/', background=False,img_name=None):
        if img_name is None:
...
def main():
 ...
 bg_remover.image(tik.openfn(),tik.backfn())
